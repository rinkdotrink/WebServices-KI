...
public class KNN
   ...
   int count;
   Perzeptron perzeptronVorfahrtVonRechts =
      new Perzeptron("Vorfahrt von Rechts");
   Perzeptron perzeptronVorfahrtGewaehren =
      new Perzeptron("Vorfahrt gewaehren");
   Perzeptron perzeptronStopp = new Perzeptron("Stop");
   Perzeptron perzeptronFahrtrichtungLinks =
      new Perzeptron("Fahrtrichtung links");
   Perzeptron perzeptronFahrtrichtungRechts =
      new Perzeptron("Fahrtrichtung rechts");
   ...
   public String classify(PropertyAndConcept propAndCon) {
      ...
      boolean vorfahrtVonRechts =
         perzeptronVorfahrtVonRechts.classify(propAndCon);
      boolean vorfahrtGewaehren =
         perzeptronVorfahrtGewaehren.classify(propAndCon);
      boolean stopp = perzeptronStopp.classify(propAndCon);
      boolean fahrtrichtungLinks =
         perzeptronFahrtrichtungLinks.classify(propAndCon);
      boolean fahrtrichtungRechts =
         perzeptronFahrtrichtungRechts.classify(propAndCon);
      boolean vorfahrtsstrasse =
         perzeptronVorfahrtsstrasse.classify(propAndCon);

      boolean sureVorfahrtVonRechts =
         vorfahrtVonRechts && !vorfahrtGewaehren && !stopp
            && !fahrtrichtungLinks && !fahrtrichtungRechts && !vorfahrtsstrasse;
      boolean sureVorfahrtGewaehren =
         !vorfahrtVonRechts && vorfahrtGewaehren && !stopp
            && !fahrtrichtungLinks && !fahrtrichtungRechts && !vorfahrtsstrasse;
      boolean sureStopp =
         !vorfahrtVonRechts && !vorfahrtGewaehren && stopp
            && !fahrtrichtungLinks && !fahrtrichtungRechts && !vorfahrtsstrasse;
      boolean sureFahrtrichtungLinks =
         !vorfahrtVonRechts && !vorfahrtGewaehren && !stopp
            && fahrtrichtungLinks && !fahrtrichtungRechts && !vorfahrtsstrasse;
      boolean sureFahrtrichtungRechts =
         !vorfahrtVonRechts && !vorfahrtGewaehren && !stopp
            && !fahrtrichtungLinks && fahrtrichtungRechts && !vorfahrtsstrasse;
      boolean sureVorfahrtsstrasse =
         !vorfahrtVonRechts && !vorfahrtGewaehren && !stopp
            && !fahrtrichtungLinks && !fahrtrichtungRechts && vorfahrtsstrasse;
      ...
      if (sureVorfahrtVonRechts) {
         ...
         System.out.println(" vorfahrtVonRechts: " + vorfahrtVonRechts
            + "vorfahrtGewaehren: " + vorfahrtGewaehren + "stopp: " + stopp
            + "fahrtrichtungLinks: " + fahrtrichtungLinks
            + "fahrtrichtungRechts " + fahrtrichtungRechts
            + "vorfahrtsstrasse " + vorfahrtsstrasse);
      }
